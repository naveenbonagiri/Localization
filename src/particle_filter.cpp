   ...
   */
    ...
    for (auto& MeasredObservations : observations) 
	{
        ...
        for (const auto& pred_meas : predicted) 
		{
            ...
            nearest_neighbour = dist(pred_meas.x, pred_meas.y, MeasredObservations.x, MeasredObservations.y);
        ...
        MeasredObservations.id = landmark_id;
    }
}

void ParticleFilter::updateWeights(double sensor_range, double Landmarks[], 
   ...
   */
    ...
    for (auto& particle : particles) 
	{
        ...
        UpdateParticleWeight(TransformedCoordinates, PredictionCoordinates, particle, Landmarks);
    }
}
...
void ParticleFilter::resample() {
   ...
   */
     std::vector<Particle> NewParticlesSet(num_particles);      /* vector for new particles */
...
{
	/* map particle coordiantes and variables for some temporary calculations */
    double x_p{}, y_p{}, SinOfTheta{}, CosOfTheta{}; 
    std::vector<LandmarkObs> TransformedCoordinates{};
    double x_c{}, y_c{}, obs_id{}; /* observation coordinates */

    /* storage for transformed coordinates */
    TransformedCoordinates.reserve(observations.size());

    /* retrieve x and y co-ordinates of the particle */
    x_p = particle.x;
    y_p = particle.y;

    /* caculate sine and cos of the particle */
    SinOfTheta = sin(particle.theta);
    CosOfTheta = cos(particle.theta);    

    /* Store the transformed co-ordinates */
    std::transform(observations.begin(), observations.end(), std::back_inserter(TransformedCoordinates), [&](LandmarkObs MeasredObservations) 
	{
            /* retrieve x, y coordinates of the observed measurements */
            x_c = MeasredObservations.x;
            y_c = MeasredObservations.y;
            obs_id = MeasredObservations.id;
		
      		LandmarkObs TransCoords{};
      
            /* Update data into return variable */
            TransCoords.x = x_p + CosOfTheta * x_c - SinOfTheta * y_c;
            TransCoords.y = y_p + SinOfTheta * x_c + CosOfTheta * y_c;
            TransCoords.id = obs_id;
...
std::vector<LandmarkObs> ParticleFilter::GetPredicatedCoordinates(const Map& map_landmarks, Particle& particle, double sensor_range) 
{
    ...
    for (const auto& GlobalCoordinates : map_landmarks.landmark_list) 
	{
        /* variables for landmark data */
        LandmarkObs map{};
        double x_p{}, y_p{};
        double distance{};

        /* retrieve x and y coordinates of the particle */
        x_p = particle.x;
        y_p = particle.y;

        /* calculate the distance between particle and the map landmark */
        distance = dist(x_p, y_p, GlobalCoordinates.x_f, GlobalCoordinates.y_f);

        /* is distance within the sensor range */
        if (distance < sensor_range) 
		{
            map.x = GlobalCoordinates.x_f;
            map.y = GlobalCoordinates.y_f;
            map.id = GlobalCoordinates.id_i;

            /* Update prediction coordinates structure */
...
void ParticleFilter::UpdateParticleWeight(const std::vector<LandmarkObs>& TransformedCoordinates, const std::vector<LandmarkObs>& PredictionCoordinates,
       Particle& particle, double Landmarks[]) {

    /* Initialize particle weight*/
    particle.weight = 1;
	
    /* Set values for multi-variate Gaussian distribution */
    auto xCovariance = Landmarks[0] * Landmarks[0];
    auto yCovariance = Landmarks[1] * Landmarks[1];
    auto gauss_norm = 1 / (2.0 * M_PI * Landmarks[0] * Landmarks[1]);

    /* Is the transformed cordinates id matching to any of the predicated coordinates */
    for (unsigned int i = 0; i < TransformedCoordinates.size(); i++) 
	{
        for (unsigned int j = 0; j < PredictionCoordinates.size(); j++) 
		{
			...
			{
                auto DiffInX = TransformedCoordinates[i].x - PredictionCoordinates[j].x;
                auto DiffInY = TransformedCoordinates[i].y - PredictionCoordinates[j].y;

                particle.weight *= exp(-(DiffInX * DiffInX / (2 * xCovariance) + DiffInY * DiffInY / (2 * yCovariance))) * gauss_norm;
            }
        }
    }
}
