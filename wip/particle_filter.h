...
class ParticleFilter {  
 public:
   ...
   */
  std::vector<LandmarkObs> TransformCarMeasToMapCoordinates(const std::vector<LandmarkObs>& observations, Particle &particle);

  /**
   * Used for associating these transformed observtions with the nearest landmark on the map
   */
  std::vector<LandmarkObs> GetPredicatedCoordinates(const Map& map_landmarks, Particle& particle, double sensor_range);

  /**
   * Used for updating weight particle by particle
   */
  void UpdateParticleWeight(const std::vector<LandmarkObs>& transformed_cordinates, const std::vector<LandmarkObs>& GlobalCoordinates, 
